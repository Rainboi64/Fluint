...
float4 Blur_Gaussian(float2 uv, Texture2D sourceTexture, SamplerState bilinearSampler, float2 texelSize,
                     float2 direction, float sigma)
{
...
float4 Blur_GaussianBilateral(float2 uv, Texture2D sourceTexture, Texture2D depthTexture, Texture2D normalTexture,
                              SamplerState bilinearSampler, float2 texelSize, float2 direction, float sigma)
{
    float weightSum = 0.0f;
    float4 color = 0.0f;
