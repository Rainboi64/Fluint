...
float3 ImageBasedLighting(Material material, float3 normal, float3 camera_to_pixel, Texture2D tex_environment,
                          Texture2D tex_lutIBL, SamplerState samplerLinear)
{
    ...
    float3 prefilteredColor = SampleEnvironment(samplerLinear, tex_environment, DirectionToSphereUV(reflection),
                                                mipLevel);
