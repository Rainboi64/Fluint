float3 LumaSharpen(float2 texCoord, Texture2D sourceTexture, SamplerState bilinearSampler, float2 resolution,
                   float sharp_strength, float sharp_clamp)
{
    /*
        LumaSharpen 1.4.1
...
#define CoefLuma float3(0.2126f, 0.7152f, 0.0722f)      // BT.709 & sRBG luma coefficient (Monitors and HD Television)
    ...
    float3 blur_ori = sourceTexture.Sample(bilinearSampler, texCoord + float2(px, -py) * 0.5f * offset_bias).rgb;
    // South East
    blur_ori += sourceTexture.Sample(bilinearSampler, texCoord + float2(-px, -py) * 0.5f * offset_bias).rgb;
    // South West
    ...
    float4 sharp_strength_luma_clamp = float4(sharp_strength_luma * (0.5f / sharp_clamp), 0.5f);
    //Roll part of the clamp into the dot

    float sharp_luma = clamp((dot(float4(sharp, 1.0f), sharp_strength_luma_clamp)), 0.0f, 1.0f);
